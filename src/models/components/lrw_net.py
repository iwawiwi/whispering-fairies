        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.normal_(
                    0, math.sqrt(2.0 / n)
                )  # init weight with normal distribution
...
class LRWVideoModel(nn.Module):
    def __init__(self, border=False, se=False, n_class=500, dropout=0.5):
        super().__init__()
        ...
        # gru
        self.gru = nn.GRU(
            in_dim, 1024, num_layers=3, batch_first=True, bidirectional=True, dropout=0.2
        )  # TODO: why dropout=0.2?
